...
class Complete {
    ...
    // Function for finding the maximum value pair
    public static int helpSanta(int n, int m, int[][] g) {
        ...
        // Build adjacency list from given graph
        for (int i = 0; i < m; i++) {
            int u = g[i][0];
            int v = g[i][1];
            adj.get(u).add(v);
        ...
        return primes.get(max - 1); // Subtract 2 instead of 1 because primes list is 0-based
    }
